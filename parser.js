...
function omit(keys, obj) {
  return Object.entries(obj).filter(([k, v]) => !keys.includes(k)).reduce((acc, [k, v]) => ({...acc, [k]: v}), {})
}
...
function parse(tokens) {
  ...
  function or(...args) {
    ...
    throw new Error(`or parser: matched none of the rules: ${args.map(fn => fn.name).join(' | ')}`)
  }
  ...
  function stateWithMoreDetails() {
    ...
    const transitionsAndStates = zeroOrMore(() => {
      return or(transition, stateParser);
    });
    zeroOrOne(() => {
      oneOrMore(newline());
      return dedent();
    ...
    const transitions = transitionsAndStates.filter(ts => ts.type === 'transition')
    const nestedStates = transitionsAndStates.filter(ts => ts.type !== 'transition');

    return {
      [stateName]: {
        type: parallel.length > 0 ? 'parallel' : 'sequential',
        on: transitions.length > 0 ? omit(['type'], arrayOfObjToObj(transitions)) : undefined,
        states: nestedStates.length > 0 ? arrayOfObjToObj(nestedStates) : undefined,
      }
    }
  }
  ...
  function stateParser() {
    try {
      const stateInfo = or(stateWithMoreDetails, stateWithNameOnly);
